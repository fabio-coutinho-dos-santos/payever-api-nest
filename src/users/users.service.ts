...
export class UsersService {
        ...
        try {
          ...
          if (e.response.status == this.STATUS_CODE_NOT_FOUND) {
            ...
            throw new InternalServerErrorException(
              MessageHelper.ERROR_ON_GET_USER_AVATAR,
            );
          }
        }
      }
  ...
  async create(user: User, image: Express.Multer.File) {
    ...
    try {
      ...
      const userUpdated = await this.usersRepository.findOneAndUpdate(
        { _id: userCreated._id },
        userCreated,
      );
      ...
      console.error(e);
      throw new InternalServerErrorException(
        MessageHelper.ERROR_ON_CREATE_NEW_USER_MESSAGE,
      );
    }
  }
  ...
  async getAvatarUserFromDatabase(id) {
    const user = await this.usersRepository.findOne({ id });
    if (user) {
      const imageBuffer = await this.readImage(
        this.UPLOAD_FOLDER_PATH + '/' + user.avatar + '.png',
      );
  ...
  async getAvatarUserFromReqresServer(id) {
    ...
    if (user) {
      try {
        ...
        const imageLocal = await this.readImage(
          this.UPLOAD_FOLDER_PATH + '/' + user.id + '-' + hash + '.png',
        );
        ...
        console.error(e);
        throw new InternalServerErrorException(
          MessageHelper.INTERNAL_SERVER_ERROR_MESSAGE,
        );
      }
    }
  }
  ...
  async delete(id) {
    ...
    if (!userStored) {
      ...
      const avatarPath =
        this.UPLOAD_FOLDER_PATH + '/' + userStored.avatar + '.png';
      const userDeleted = await this.usersRepository.deleteMany({
        id: userStored.id,
      });
      try {
